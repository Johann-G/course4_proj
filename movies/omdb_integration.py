...
def fill_movie_details(movie):
    ...
    movie.save()
    from movies.signals import movie_filled
...
def search_and_save(search):
    ...
    if not created and (search_term.last_search > now() - timedelta(minutes=1)):
        logger.warning("Search for '%s' was performed in the past minute so not searching again.", normalized_search_term)
        return
    omdb_client = get_client_from_settings()
